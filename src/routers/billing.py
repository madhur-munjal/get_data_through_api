...
def create_billing(
    ...
    try:
        ...
        appointment_db = (
            db.query(Appointment).filter_by(id=billing_details.appointment_id).first()
        )
        ...
        notification_data = {
            ...
            "billing_id": created_billing_id,
            "firstName": appointment_db.patient.firstName,
...
def get_billing_details(
    ...
    type: str = Query(
        None, description="what type of transaction, cash, card, upi etc."
    ),
    ...
    if type:
        query = query.filter(
            Billing.type.ilike(type)
        )  # (func.lower(Billing.type) == type.lower())
...
def get_billing_summary(
    ...
    try:
        total_earning = query.with_entities(
            func.coalesce(func.sum(Billing.amount), 0)
        ).scalar()
        completed_payment_ids = query.filter(
        ...
        payment_details = (
            ...
            .group_by(Billing.type)
            .having(func.sum(Billing.amount) > 0)
            .all()
        )
        ...
        final_result = [
            {"type": t, "total": payment_summary.get(t, 0)} for t in fixed_types
        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching billing summary: {str(ex)}"
        )
...
def soft_delete_billing(
    ...
    for appointment_id in appointment_ids:
        active_billing_exists = (
            db.query(Billing)
            .filter(
                Billing.appointment_id == appointment_id, Billing.is_deleted == False
            )
            .first()
        )
    ...
    if rows_updated == 0:
        raise HTTPException(
            status_code=404, detail="No billing records found for given IDs"
        )
